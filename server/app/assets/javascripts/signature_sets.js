...
window.types = ['Ignore', 'Integer', 'Range', 'C string', 'WC string', 'Pointer', 'Bitmask', 'Blob', 'Not'];
...
window.addParam = function(myVal, arg) {
	...
	$('<td><select id="sel'+currentNum+'" name="type'+currentNum+'" onchange="typeChange('+currentNum+')"><option value="Integer">Integer</option><option value="C string">C string</option><option value="WC string">Wide-char string</option><option value="Blob">Blob</option><option value="Pointer">Pointer</option></select></td>').insertAfter($('#typerow').find('td:last'));
...
function typeChange(i,arg){
	...
	}else if(val == "C string" || val == "WC string"){
